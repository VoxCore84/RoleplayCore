# fan-out-scaling — reference (moved from .claude/rules/fan-out-scaling.md on 2026-09-22; operative rules remain in the rules file)

## Provenance preamble (moved verbatim)

Every numeric claim below was verified against live docs on
...
## Mode-table "Why" column and mode narrative (moved verbatim)
...
## Seven primitives — full narrative annotations (moved verbatim)
...
1. **Bounded concurrency.** A semaphore/worker-pool caps concurrency but does **not by itself** cap the
   ...
   (Mode B's ≤16 cap is this primitive.)
2. **Exponential backoff with full jitter.** Full jitter
   **substantially reduces** (does not *eliminate*) synchronized-retry thundering herds.
3. **Failure isolation (bulkhead).** Report "X/N succeeded, Y failed", write the failure list for triage.
4. **Per-result durability.** Crash loss is bounded by *in-flight* count (≈pool size), not 1, and not the whole batch.
6. **Staggered ramp + canary.** (Mode B queues ≤16 at a time = inherent ramp.)
7. **Live observability.** ETA (`elapsed/done * remaining`). In Mode A, overwrite a `STATUS.txt` the operator can `tail` from another terminal.
...
## Preflight gate — full text of the [1m] gate entry (moved verbatim)
...
## Corrections added 2026-09-22 (optimization sweep)

- ~~"Sub-agents get fresh context, don't inherit parent conversation (fork is the exception)"~~ **[Correction 2026-09-22]:** the changelog for v2.1.232 reportedly made fork-mode subagents the default in interactive sessions (`CLAUDE_CODE_FORK_SUBAGENT`), per the env-audit claude-code-guide pass (`AI_Studio/Reports/sweep_2026-09-22/1a_settings_audit.md`). Sourced from the changelog digest, not re-read on the live sub-agents page → UNVERIFIED in detail; the operative guidance (Mode A/B/C, canary, unique output paths) is unaffected. Verify before quoting cache-sharing behaviour for non-fork agent types.
- The 2026-09-22 canary (3 general-purpose subagents, no model param) self-reported `claude-sonnet-5` with no credit-gate prompt — evidence for the "[1m] gate RESOLVED" line (`00_phase0_evidence.md` § 0.5).
