...
- ~~**Eval sessions load the global CLAUDE.md.** Any before/after eval comparison must record its SHA-256; a block edit moves eval results.~~ **[Correction 2026-09-24, session 290]:** false. A probe case run through `claude plugin eval` (`superpowers_activation/evals/adam3-inherit-check`) answered NONE in both arms when asked to quote the block; the docs state eval children load no CLAUDE.md files. The 4/6 → 3/3 react-todo change credited to the line-9 amendment was variance. Original preserved as audit trail; see `tasks/lessons.md` 2026-09-24.
