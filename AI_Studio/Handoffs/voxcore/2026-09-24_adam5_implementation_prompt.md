...
3. **`tools/activation/eval_run.ps1`**: ALREADY DONE at the session-290 wrap-up — the PowerShell loop was replaced by a call to `tools/activation/copy_traces.py` and verified ("traces copied: 2 (missing 0)" on `adam4-copier-check`). Just confirm the line is present; skip the rest of this item.
...
7. **VoxCore `.claude/agents/readonly-reviewer.md`**: ALREADY CREATED at the session-290 wrap-up (tools Read, Grep, Glob; effort high; no model pin). Agent types register at session start, so it is available to you as `readonly-reviewer`; confirm it appears in your agent list (a dispatch with "list your tools" is enough) and use it for every review agent in Phase 3. This is what Part D lacked (11 of 95 agents used Bash despite a prose ban).
