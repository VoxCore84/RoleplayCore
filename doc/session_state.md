...
| **Mbox-Fast + Recordings** | (1) Built `tools/mbox/parallel.py` — 30s full 9.8 GB Gmail index (~7× over single-thread). Live DB: `Desktop/Excluded/mbox/mbox_index.db`, 17,050 msgs / 3,248 atts / FTS5 trigram. (2) Transcribing 24 `.m4a` files from `Desktop/Excluded/Recordings/` via `audio_transcribe_v2.py --model medium --device cuda`. Cache: `.cache/transcribed/Recordings_816ff3c0e3/`. RAG ingest via `rag_build.py` queued for after transcription. | ACTIVE | Owns: `tools/mbox/*`, `Desktop/Excluded/mbox/mbox_index.db`, `.cache/transcribed/Recordings_*/`. Does NOT touch: `Case_Reference/`, `memory/case-*.md`, `tools/rag_*.py` (read-only use), `tools/audio_transcribe*.py` (read-only use). Background jobs: `b95foj3mq` (transcription). |
