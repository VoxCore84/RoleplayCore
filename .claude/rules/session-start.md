...
1. **Read `docs/VOXCORE_HANDOFF_INDEX.md` FIRST — the canonical entry point.** It carries current state + the latest-session pointer + `AI_Studio/Handoffs/voxcore/CURRENT_STATE.md` and `NEXT_SESSION.md`. It **supersedes** `AI_Studio/0_Central_Brain.md` and `doc/session_state.md` where they conflict (both can be stale). Then skim `0_Central_Brain.md` for Triad context. For deep cross-tab reconciliation (git/branch ownership, memory-repo safety, blocker detection), run **`/start-up`**.
